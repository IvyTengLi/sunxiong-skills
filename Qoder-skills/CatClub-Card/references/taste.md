...
- 段落文本颜色避免纯黑，用 `#1A1A1A` 或 `#4A4A4A` 等深灰
...
### 品牌背景色偏移上限
- 品牌基准背景色为 Carrara 冷白 `#F0F0F0`
- 根据内容气质微调背景色时，偏移幅度控制在 +/- 8 hex 以内
- 禁止使用暖黄/暖橙等明显偏暖的背景（保持 Carrara 大理石“冷白”基调）
- 背景色 + 强调色双色对的具体色板见 mode-long.md 步骤 2.5
...
- [ ] 是否避免了三等分等宽卡片？
- [ ] 标题是否用了非 Inter 字体（Noto Serif SC / Noto Sans SC / JetBrains Mono）？
- [ ] 颜色是否统一冷暖调，无纯黑？
- [ ] 强调色是否 ≤ 1 个且饱和度 < 80%？
